...
const (
	...
	// 展厅需求单状态（与 Java StateCode SUPPLY_TASK_* 一致）
	SUPPLY_TASK_MATCH  uint = 1000 // 匹配中
	SUPPLY_TASK_FINISH uint = 1010 // 已完成
	SUPPLY_TASK_CANAL  uint = 1020 // 已取消
	...
	// 微信小程序发货信息录入 trade_order_info.order_push_wx_shipping
	ORDER_PUSH_WX_SHIPPING_WAIT int = 0 // 待推送
	ORDER_PUSH_WX_SHIPPING_OK   int = 1 // 已上传成功
	ORDER_PUSH_WX_SHIPPING_FAIL int = 2 // 上传失败
	ORDER_PUSH_WX_SHIPPING_SKIP int = 3 // 跳过（非微信支付、无支付单等，不再重试）

	// 微信小程序确认收货提醒 trade_order_info.order_received_push_wx（依赖发货推送成功后再处理）
	ORDER_RECEIVED_PUSH_WX_WAIT int = 0 // 待推送
	ORDER_RECEIVED_PUSH_WX_OK   int = 1 // 推送成功
	ORDER_RECEIVED_PUSH_WX_FAIL int = 2 // 推送失败
	ORDER_RECEIVED_PUSH_WX_SKIP int = 3 // 跳过（非实物、非快递配送、非微信支付等）
	...
	*/
	...
	TRADE_TYPE_BUY_PLUS   uint = 1507 //PLUS会员卡购买

	PAYMENT_TYPE_DELIVER uint = 1301 //货到付款
	PAYMENT_TYPE_ONLINE  uint = 1302 //在线支付
	PAYMENT_TYPE_CREDIT  uint = 1303 //白条/信用支付
	//PAYMENT_TYPE_CASH    uint  = 1304//现金支付
	PAYMENT_TYPE_OFFLINE uint = 1305 //线下支付
	PAYMENT_TYPE_BANK    uint = 1306 //银行打款（与 Java StateCode.PAYMENT_TYPE_BANK 一致）
	...
	// USER_PLUS_TYPE_TRIAL PLUS 体验卡类型（与 Java UserPlusOrderServiceImpl 固定 plus_type_id=1001 一致，限购一次）
	USER_PLUS_TYPE_TRIAL uint = 1001

	// 券领取来源字符串（与 Java StateCode.VOUCHER_SOURCE_* 一致）
	VOUCHER_SOURCE_DEFAULT      = "normal"
	VOUCHER_SOURCE_MANUAL       = "manual"
	VOUCHER_SOURCE_PLUS_OPEN    = "open"
	VOUCHER_SOURCE_PLUS_MONTHLY = "monthly"
	VOUCHER_SOURCE_ADMIN        = "admin"
	VOUCHER_SOURCE_FULL_RETURN  = "full_return"

	// 价格类型（与 Java StateCode.PRICE_TYPE_* 一致）
	PRICE_TYPE_NORMAL   = "normal"
	PRICE_TYPE_LEVEL    = "level"
	PRICE_TYPE_PLUS     = "plus"
	PRICE_TYPE_ACTIVITY = "activity"
	PRICE_TYPE_POLICY   = "policy"
)
