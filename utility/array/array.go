...
// Column 提取结构体切片中指定字段路径的值，保持顺序并去重
//
//	itemIds := gconv.SliceUint64(array.ColumnT[*model.CheckoutItemVo, uint64](in.Items, "ItemId"))
func ColumnT[T any, V comparable](slice []T, columnPath string) []V {
	// 转换为驼峰路径（兼容 snake_case 字段）
	parts := strings.Split(columnPath, ".")
	for i, p := range parts {
		parts[i] = gstr.CaseCamel(p)
	}
	...
	for _, item := range slice {
		...
		if valid && val.CanInterface() {
			if v, ok := val.Interface().(V); ok {
				if _, exists := seen[v]; !exists {
					seen[v] = struct{}{}
