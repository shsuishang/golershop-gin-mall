...
func Lk(ctx context.Context) string {
	url, _ := g.Cfg().Get(ctx, "licence.key")
	if url.String() != "" {
		return url.String()
	} else {
	}
...
// Separator
func Separator(ctx context.Context) string {
	separator, _ := g.Cfg().Get(ctx, "shopSuite.cacheSeparator")
	if separator.String() != "" {
